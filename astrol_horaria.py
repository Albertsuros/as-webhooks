# ASTROLOGÍA HORARIA - Carta para momento de pregunta
# Basado en carta_natal.py pero adaptado para preguntas horarías
...
class CartaAstralNatal:
    ...
    def guardar_carta_natal_con_nombre_unico(self, fecha_natal, directorio_salida):
        ...
        if hasattr(self, 'nombre_archivo_personalizado') and self.nombre_archivo_personalizado:
            ...
            nombre_base = f"astrol_horaria_{año_natal}{mes_natal:02d}{dia_natal:02d}_{hora_natal:02d}{min_natal:02d}"
        ...
        try:
            ...
            print(f"✓ Carta de astrología horaria guardada como: {ruta_completa}")
...
def main(nombre_archivo_salida=None, datos_horaria=None):
    """Función principal para generar astrología horaria"""
    print("=== GENERADOR DE ASTROLOGÍA HORARIA ===")
    print("Calculando carta para momento de pregunta...")
    
    # Datos por defecto si no se proporcionan
    if datos_horaria:
        fecha_pregunta = datos_horaria.get('fecha_pregunta', None)
        lugar_pregunta = datos_horaria.get('lugar_pregunta', (40.42, -3.70))
        pregunta = datos_horaria.get('pregunta', "Pregunta no especificada")
    else:
        # Si no se especifica fecha, usar momento actual
        hoy = datetime.now()
        fecha_pregunta = (hoy.year, hoy.month, hoy.day, hoy.hour, hoy.minute)
        lugar_pregunta = (40.42, -3.70)  # Madrid por defecto
        pregunta = "Pregunta no especificada"
    
    print(f"Fecha de pregunta: {fecha_pregunta[2]:02d}/{fecha_pregunta[1]:02d}/{fecha_pregunta[0]} {fecha_pregunta[3]:02d}:{fecha_pregunta[4]:02d}")
    print(f"Pregunta: {pregunta}")
    
    try:
        # Crear instancia de la clase
        carta = CartaAstralNatal(figsize=(14, 12))
        
        if nombre_archivo_salida:
            carta.nombre_archivo_personalizado = nombre_archivo_salida
        
        # Generar la carta horaria
        aspectos, posiciones = carta.crear_carta_astral_natal(
            fecha_natal=fecha_pregunta,
            lugar_natal=lugar_pregunta,
            ciudad_natal="Madrid, España",
            guardar_archivo=True,
            directorio_salida="static"
        )
        
        print(f"\n✓ Carta de astrología horaria generada exitosamente")
        print(f"Planetas calculados: {len(posiciones)}")
    ...
    for i, persona in enumerate(personas, 1):
        try:
            ...
            aspectos, posiciones = carta.crear_carta_astral_natal(
                ...
                guardar_archivo=True,
                directorio_salida="cartas_natales_multiples"
            )
...
def generar_carta_natal_personalizada():
    ...
    try:
        ...
        aspectos, posiciones = carta.crear_carta_astral_natal(
            ...
            guardar_archivo=True,
            directorio_salida="cartas_natales_personalizadas"
        )
