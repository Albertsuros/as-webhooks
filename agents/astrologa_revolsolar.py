...
def enviar_a_make(datos):
    try:
        url = os.getenv("MAKE_WEBHOOK_ASTROLOGA_REVOLSOLAR")
        ...
        if not url:
            print("❌ ERROR: No hay URL de Make configurada para MAKE_WEBHOOK_ASTROLOGA_REVOLSOLAR")
            print("Verifica que MAKE_WEBHOOK_ASTROLOGA_REVOLSOLAR esté en tu archivo .env")
            return False
        ...
        headers = {
            ...
            'User-Agent': 'AS-Asesores-Astrologa-Revolsolar/1.0'
        }
...
def handle_astrologa_revolsolar_webhook(data):
    try:
        session_id = data.get("session_id")
        mensaje_usuario = data.get("text", "")
        print(f"=== WEBHOOK ASTROLOGA REVOLSOLAR ===")
        ...
        print(f"Datos actuales (Astrologa - Revolsolar): {datos}")
        ...
        if completos:
            ...
            if exito_envio:
                # ✅ GENERAR INFORME ANTES DE ELIMINAR SESIÓN:
                resumen = f"Resumen completo de 50 minutos sobre carta astral y revolución solar con todos los datos proporcionados durante la consulta."
                resultado = generar_y_enviar_informe_desde_agente(data, 'revolucion_solar_ia', resumen)
        ...
        print(f"❌ Error en handle_astrologa_revolsolar_webhook: {e}")
        import traceback
        traceback.print_exc()
