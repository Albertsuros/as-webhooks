...
def completar_datos(session_id, mensaje):
    ...
    if "psicologial" in mensaje_lower or "coaching" in mensaje_lower:
        servicio_detectado = "psico-coaching"
...
def enviar_a_make(datos):
    try:
        url = os.getenv("MAKE_WEBHOOK_PSICO_COACHING")
        ...
        if not url:
            print("❌ ERROR: No hay URL de Make configurada para MAKE_WEBHOOK_PSICO_COACHING")
            print("Verifica que MAKE_WEBHOOK_PSICO_COACHING esté en tu archivo .env")
            return False
        ...
        headers = {
            ...
            'User-Agent': 'AS-Cartastral-Psico_Coaching/1.0'
        }
...
def handle_psico_coaching_webhook(data):
    try:
        session_id = data.get("session_id")
        mensaje_usuario = data.get("text", "")

        print(f"=== WEBHOOK PSICO_COACHING ===")
        ...
        print(f"Datos actuales (Psico-Coaching): {datos}")
        ...
        print(f"❌ Error en handle_psico_coaching_webhook: {e}")
        import traceback
        traceback.print_exc()
