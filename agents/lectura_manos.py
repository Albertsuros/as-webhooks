...
def completar_datos(session_id, mensaje):
    ...
    # ✅ CORRECCIÓN 2: Detectar servicios lectura manos
    servicio_detectado = None
    
    if "lectura" in mensaje_lower or "manosl" in mensaje_lower:
        servicio_detectado = "Lectura Manos"
        ...
        servicio_detectado = "Lectura Manos"  # Por defecto
...
def enviar_a_make(datos):
    try:
        url = os.getenv("MAKE_WEBHOOK_LECTURA_MANOS")
        
        # ✅ CORRECCIÓN 3: Debugging mejorado
        print(f"=== DEBUG ENVIAR_A_MAKE (LECTURA MANOS) ===")
        print(f"URL desde .env: {url}")
        print(f"Datos a enviar: {datos}")
        
        if not url:
            print("❌ ERROR: No hay URL de Make configurada para MAKE_WEBHOOK_LECTURA_MANOS")
            print("Verifica que MAKE_WEBHOOK_LECTURA_MANOS esté en tu archivo .env")
            return False
        ...
        headers = {
            ...
            'User-Agent': 'AS-Asesores-Lectura-Manos/1.0'
        }
...
def responder_ia(mensaje_usuario, datos_actuales=None):
    try:
        ...
        prompt = f"""Eres Sofía, la lectora de manos virtual de AS Carta Astral. Atiendes llamadas entrantes con una voz cálida y cercana para recoger los datos necesarios para generar una carta astral o interpretación personalizada.
...
def handle_lectura_manos_webhook(data):
    try:
        session_id = data.get("session_id")
        mensaje_usuario = data.get("text", "")

        print(f"=== WEBHOOK LECTURA MANOS ===")
        ...
        print(f"Datos actuales (Lectura - Manos): {datos}")
        ...
        if completos:
            ...
            if exito_envio:
                ...
                resumen = f"Resumen completo de 30 minutos sobre lectura de manos con todos los datos proporcionados durante la consulta."
                resultado = generar_y_enviar_informe_desde_agente(data, 'lectura_manos_ia', resumen)
                
                return {"type": "speak", "text": "Gracias. Ya tengo toda la información. En breve recibirás tu interpretación de la lectura de manos."}
            else:
                return {"type": "speak", "text": "Gracias por la información. Estamos procesando tu solicitud lectura de manos y nos contactaremos contigo pronto."}
        else:
            respuesta = responder_ia(mensaje_usuario, datos)
            return {"type": "speak", "text": respuesta}

    except Exception as e:
        print(f"❌ Error en handle_lectura_manos_webhook: {e}")
        import traceback
        traceback.print_exc()
