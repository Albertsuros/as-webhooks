...
def enviar_a_make(datos):
    try:
        url = os.getenv("MAKE_WEBHOOK_VENDEDOR_1")
        if url:
            response = requests.post(url, json=datos, timeout=10)
            print(f"Enviado a Make (Vendedor 1): {response.status_code}")
            print(f"Datos enviados: {datos}")
        else:
            print("No hay URL de Make configurada para Vendedor 1")
...
def responder_ia(mensaje_usuario, datos_actuales=None):
    try:
        ...
        prompt = f"""Eres el Vendedor 1 de AS Asesores. Atiendes llamadas entrantes de forma profesional y cercana para recopilar los datos necesarios para ofrecer asesoramiento en inteligencia artificial a negocios.
...
def handle_vendedor1_webhook(data):
    try:
        ...
        print(f"Datos actuales (Vendedor 1): {datos}")
        ...
        print(f"Error en handle_vendedor1_webhook: {e}")
