...
def enviar_a_make(datos):
    try:
        url = os.getenv("MAKE_WEBHOOK_BUSCA_EMPRESAS2")
        if url:
            response = requests.post(url, json=datos, timeout=10)
            print(f"Enviado a Make (Busca empresas2): {response.status_code}")
            print(f"Datos enviados: {datos}")
        else:
            print("No hay URL de Make configurada para Busca empresas2")
...
def handle_busca_empresas2_webhook(data):
    try:
        ...
        print(f"Datos actuales (Busca empresas2): {datos}")
        ...
        print(f"Error en handle_busca_empresas2_webhook: {e}")
