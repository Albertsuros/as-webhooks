...
def enviar_a_make(datos):
    try:
        url = os.getenv("MAKE_WEBHOOK_REDES_SOCIALES6")
        if url:
            response = requests.post(url, json=datos, timeout=10)
            print(f"Enviado a Make (Redes Sociales6): {response.status_code}")
            print(f"Datos enviados: {datos}")
        else:
            print("No hay URL de Make configurada para Redes Sociales6")
...
def responder_ia(mensaje_usuario, datos_actuales=None):
    try:
        ...
        prompt = f"""Eres el agente de redes sociales número 6 de AS Asesores. Atiendes llamadas o chats para captar empresas interesadas en la gestión profesional de sus redes sociales y otros servicios relacionados con inteligencia artificial.
...
def handle_redes_sociales6_webhook(data):
    try:
        ...
        print(f"Datos actuales (Redes Sociales6): {datos}")
        ...
        print(f"Error en handle_redes_sociales6_webhook: {e}")
