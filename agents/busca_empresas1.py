...
def completar_datos(session_id, mensaje):
    ...
    # Servicio de interés - CORREGIDO
    servicio_detectado = None

    if any(palabra in mensaje_lower for palabra in ["buscar", "empresas", "empresa", "busqueda"]):
        servicio_detectado = "Busca empresas"
    elif "redes sociales" in mensaje_lower or "redes" in mensaje_lower:
...
def enviar_a_make(datos):
    try:
        url = os.getenv("MAKE_WEBHOOK_BUSCA_EMPRESAS1")
        if url:
            response = requests.post(url, json=datos, timeout=10)
            print(f"Enviado a Make (Busca empresas1): {response.status_code}")
            print(f"Datos enviados: {datos}")
        else:
            print("No hay URL de Make configurada para Busca empresas1")
...
def responder_ia(mensaje_usuario, datos_actuales=None):
    try:
        ...
        prompt = f"""Eres un agente buscador de empresas de AS Asesores. Te encargas de encontrar y registrar empresas potencialmente interesadas en servicios de inteligencia artificial. Recibes información por llamada o chat, y debes recopilar los siguientes datos:

- Nombre de la persona de contacto
- Número de teléfono
- Email
- Tipo de servicio que puede interesar (automatización, redes sociales, asesoría en IA, etc.)

Datos recopilados hasta ahora:
{contexto if contexto else "Ninguno"}

Si ya tienes todos los datos, agradece y finaliza la conversación. Responde en el mismo idioma que el cliente.
...
def handle_busca_empresas1_webhook(data):
    try:
        ...
        print(f"Datos actuales (Busca empresas1): {datos}")
        print(f"Completos: {completos}")
        
        if completos:
            enviar_a_make(datos)
            sessions.pop(session_id, None)
            return {"type": "speak", "text": "Gracias. Ya he anotado todos los datos. Nuestro equipo revisará esta empresa pronto."}
        else:
            respuesta = responder_ia(mensaje_usuario, datos)
            return {"type": "speak", "text": respuesta}

    except Exception as e:
        print(f"Error en handle_busca_empresas1_webhook: {e}")
