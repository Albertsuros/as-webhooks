...
def manejar_save_lead_vendedor3(data):
    """Personalizar save_lead para Vendedor 3"""
    # Añadir información específica del vendedor
    data['agente'] = 'Vendedor 3 - Comercial'
...
def enviar_a_make(datos):
    try:
        url = os.getenv("MAKE_WEBHOOK_VENDEDOR_3")
        if url:
            response = requests.post(url, json=datos, timeout=10)
            print(f"Enviado a Make (Vendedor 3): {response.status_code}")
            print(f"Datos enviados: {datos}")
        else:
            print("No hay URL de Make configurada para Vendedor 3")
    except Exception as e:
...
def responder_ia(mensaje_usuario, datos_actuales=None):
    try:
        ...
        prompt = f"""Eres el Vendedor 3 de AS Asesores. Atiendes llamadas entrantes de forma profesional y cercana para recopilar los datos necesarios para ofrecer asesoramiento en inteligencia artificial a negocios.
...
def handle_vendedor3_webhook(data):
    try:
        ...
        print(f"Datos actuales (Vendedor 3): {datos}")
        ...
        print(f"Error en handle_vendedor3_webhook: {e}")
