...
def obtener_zone_id_desde_coordenadas(latitud, longitud):
    ...
    try:
        ...
        # CONVERTIR coordenadas al formato de la BD (FUERA de la consulta SQL)
...
class CartaRevolucionSolar:
    ...
    def dibujar_divisiones_casas(self, cuspides_casas, ascendente, es_revolucion=False):
        ...
        for i, cuspide in enumerate(cuspides_casas):
            ...
            # Número de casa solo para carta natal
            if not es_revolucion:
    ...
    def dibujar_ascendente_mediocielo(self, ascendente, mediocielo, es_revolucion=False, ascendente_referencia=None):
        """Dibujar Ascendente y Mediocielo"""
        if es_revolucion:
            color_asc = 'darkred'
            color_mc = 'darkmagenta'
            prefijo = 'RS-'
            linewidth = 2.5
            y_pos = 1.25
            fontsize = 7
        ...
        if es_revolucion and ascendente_referencia is not None:
            # Revolución Solar: usar ascendente natal como referencia
    ...
    def dibujar_planetas_revolucion_solar(self, pos_natales, pos_revolucion, ascendente):
        """Dibujar planetas natales y de revolución solar"""
        ...
        # Planetas de revolución solar en círculo externo (radio 0.82)
        for planeta, data in pos_revolucion.items():
            ...
            # Símbolo revolución solar (círculo externo)
            self.ax.text(rad, 0.82, simbolo, ha='center', va='center', 
                        fontsize=18, weight='bold', color=color)
            
            # Información revolución solar (arriba del símbolo)
            grado_en_signo = grado % 30
            signo_idx = int(grado // 30)
            signo = NOMBRES_SIGNOS[signo_idx]
    ...
    def calcular_aspectos_revolucion_solar(self, pos_natales, pos_revolucion, ascendente_natal, mediocielo_natal, ascendente_rs, mediocielo_rs):
        """Calcular aspectos entre planetas natales y de revolución solar Y también a ASC/MC"""
        aspectos = []
        ...
        # ASPECTOS PLANETA-PLANETA (RS a Natal)
        for planeta_rs, data_rs in pos_revolucion.items():
            for planeta_natal, data_natal in pos_natales.items():
                grado_rs = data_rs['grado']
                grado_natal = data_natal['grado']
                
                # Calcular diferencia angular
                diferencia = abs(grado_rs - grado_natal)
                if diferencia > 180:
        ...
        # ASPECTOS PLANETA RS - ASC/MC NATAL
        puntos_angulares_natal = {
            'Ascendente': ascendente_natal,
            'Mediocielo': mediocielo_natal
        }
        
        for planeta_rs, data_rs in pos_revolucion.items():
            for punto_angular, grado_angular in puntos_angulares_natal.items():
                grado_rs = data_rs['grado']
                
                # Calcular diferencia angular
                diferencia = abs(grado_rs - grado_angular)
                if diferencia > 180:
                    diferencia = 360 - diferencia
                ...
                for aspecto, angulo in angulos_aspectos.items():
                    ...
                    if orbe <= tolerancia:
                        aspectos.append({
                            ...
                            'planeta_rs': planeta_rs,
                            'planeta_natal': f"{punto_angular} Natal",
                            'aspecto': aspecto,
                            'orbe': orbe,
                            'grado_rs': grado_rs,
        ...
        # ASPECTOS PLANETA NATAL - ASC/MC RS
        puntos_angulares_rs = {
        ...
        for planeta_natal, data_natal in pos_natales.items():
            for punto_angular, grado_angular in puntos_angulares_rs.items():
                grado_natal = data_natal['grado']
                
                # Calcular diferencia angular
                diferencia = abs(grado_natal - grado_angular)
                if diferencia > 180:
                    diferencia = 360 - diferencia
                ...
                for aspecto, angulo in angulos_aspectos.items():
                    ...
                    if orbe <= tolerancia:
                        aspectos.append({
                            'tipo_aspecto': 'planeta-punto',
                            'planeta_rs': f"{punto_angular} RS",
                            'planeta_natal': planeta_natal,
                            'aspecto': aspecto,
                            'orbe': orbe,
                            'grado_rs': grado_angular,
                            'grado_natal': grado_natal,
                            'diferencia_angular': diferencia
                        })
                        break
    ...
    def dibujar_aspectos_revolucion_solar(self, aspectos, ascendente):
        ...
        for aspecto in aspectos:
            ...
            grado_rs = aspecto['grado_rs']
            grado_natal = aspecto['grado_natal']
            
            rad_rs = self.grado_a_coordenada_carta(grado_rs, ascendente)
            rad_natal = self.grado_a_coordenada_carta(grado_natal, ascendente)
            
            color = COLORES_ASPECTOS.get(aspecto['aspecto'], 'purple')
            
            # Línea de aspecto
            self.ax.plot([rad_rs, rad_natal], [0.82, 0.68], 
                        color=color, linewidth=1.5, alpha=0.7, linestyle='-')

    def crear_carta_revolucion_solar(self, fecha_natal, lugar_natal, ciudad_natal,
                                    año_revolucion, lugar_revolucion, ciudad_revolucion,
                                    guardar_archivo=True, directorio_salida="static", nombre_archivo=None):
        """Crear la carta de revolución solar completa"""
        
        print(f"\n=== CALCULANDO REVOLUCIÓN SOLAR {año_revolucion} ===")
        ...
        # Casas de ambas cartas
        latitud_rs, longitud_rs = lugar_revolucion
        cuspides_natal, ascendente_natal, mediocielo_natal = self.calcular_casas_placidus(jd_natal, latitud_natal, longitud_natal)
        cuspides_rs, ascendente_rs, mediocielo_rs = self.calcular_casas_placidus(jd_revolucion, latitud_rs, longitud_rs)
        
        # Dibujar la carta (usando ascendente natal como referencia)
        self.dibujar_circulos_concentricos()
        self.dibujar_signos_zodiacales(ascendente_natal)
        self.dibujar_divisiones_casas(cuspides_natal, ascendente_natal, es_revolucion=False)
        self.dibujar_divisiones_casas(cuspides_rs, ascendente_natal, es_revolucion=True)
        self.dibujar_ascendente_mediocielo(ascendente_natal, mediocielo_natal, es_revolucion=False)
        self.dibujar_ascendente_mediocielo(ascendente_rs, mediocielo_rs, es_revolucion=True, ascendente_referencia=ascendente_natal)
        self.dibujar_planetas_revolucion_solar(pos_natales, pos_revolucion, ascendente_natal)
        
        # Calcular y dibujar aspectos
        aspectos = self.calcular_aspectos_revolucion_solar(pos_natales, pos_revolucion, 
                                                         ascendente_natal, mediocielo_natal, 
                                                         ascendente_rs, mediocielo_rs)
        self.dibujar_aspectos_revolucion_solar(aspectos, ascendente_natal)
        ...
        titulo = f"REVOLUCIÓN SOLAR {año_revolucion}\nNatal: {fecha_natal_str} - {ciudad_natal}\nRevolución: {fecha_rs_str} - {ciudad_revolucion}"
        ...
        # Leyenda
        self.crear_leyenda_revolucion_solar()
        
        # Guardar archivo
        if guardar_archivo:
            self.guardar_revolucion_solar_con_nombre_unico(
                fecha_natal, año_revolucion, directorio_salida
            )
        
        # MOSTRAR INFORMACIÓN COMPLETA EN CONSOLA
        self.mostrar_informacion_completa_revolucion_solar(
            pos_natales, pos_revolucion, aspectos, 
            ascendente_natal, mediocielo_natal, ascendente_rs, mediocielo_rs,
            ciudad_natal, ciudad_revolucion, fecha_revolucion
        )
        
        return aspectos, pos_natales, pos_revolucion, fecha_revolucion
    
    def mostrar_informacion_completa_revolucion_solar(self, pos_natales, pos_revolucion, aspectos,
                                                     ascendente_natal, mediocielo_natal, ascendente_rs, mediocielo_rs,
                                                     ciudad_natal, ciudad_revolucion, fecha_revolucion):
        """Mostrar información completa de la revolución solar en consola"""
        
        año_rs, mes_rs, dia_rs, hora_rs, min_rs = fecha_revolucion
        
        print(f"\n" + "="*60)
        print(f"INFORMACIÓN COMPLETA DE REVOLUCIÓN SOLAR")
        print(f"="*60)
        ...
        print(f"\n=== POSICIONES PLANETARIAS REVOLUCIÓN SOLAR ({ciudad_revolucion}) ===")
        ...
        print(f"\n=== PUNTOS ANGULARES REVOLUCIÓN SOLAR ===")
        ...
        # ASPECTOS COMPLETOS
        if aspectos:
            print(f"\n=== ASPECTOS REVOLUCIÓN SOLAR-NATAL ({len(aspectos)}) ===")
            ...
            if aspectos_planeta_planeta:
                ...
                for aspecto in aspectos_planeta_planeta:
                    print(f"• {aspecto['planeta_rs']} (RS) {aspecto['aspecto'].upper()} {aspecto['planeta_natal']} (N) - Orbe: {aspecto['orbe']:.2f}°")
            ...
            if aspectos_planeta_punto:
                ...
                for aspecto in aspectos_planeta_punto:
                    print(f"• {aspecto['planeta_rs']} {aspecto['aspecto'].upper()} {aspecto['planeta_natal']} - Orbe: {aspecto['orbe']:.2f}°")
        else:
            print("\n=== NO SE ENCONTRARON ASPECTOS SIGNIFICATIVOS ===")
        
        print(f"\n" + "="*60)
    
    def crear_leyenda_revolucion_solar(self):
        """Crear leyenda explicativa de la revolución solar"""
        leyenda_texto = [
            "LEYENDA:",
            "• Círculo interno: Planetas Natales",
            "• Círculo externo: Planetas Revolución Solar", 
            "• Líneas negras continuas: Casas Natales",
            "• Líneas rojas punteadas: Casas Revolución Solar",
            "• Líneas de colores: Aspectos RS-Natal",
            "• ℞ = Planeta retrógrado",
            "• El Sol RS coincide exactamente con el Sol natal"
        ]
    ...
    def guardar_revolucion_solar_con_nombre_unico(self, fecha_natal, año_revolucion, directorio_salida):
        ...
        if hasattr(self, 'nombre_archivo_personalizado') and self.nombre_archivo_personalizado:
            ...
            nombre_base = f"revolucion_solar_{año_natal}{mes_natal:02d}{dia_natal:02d}_{hora_natal:02d}{min_natal:02d}_RS{año_revolucion}"
        ...
        try:
            ...
            print(f"✓ Carta de revolución solar guardada como: {ruta_completa}")
            return ruta_completa
        except Exception as e:
            print(f"✗ Error guardando la carta: {e}")
            return None

# FUNCIÓN MAIN PARA REVOLUCIÓN SOLAR
def main(nombre_archivo_salida=None):
    """Función principal para generar revolución solar"""
    print("=== GENERADOR DE CARTA DE REVOLUCIÓN SOLAR ===")
    print("Calculando revolución solar...")
    ...
    try:
        ...
        carta = CartaRevolucionSolar(figsize=(16, 14))
        if nombre_archivo_salida:
            carta.nombre_archivo_personalizado = nombre_archivo_salida
        
        # Generar la carta de revolución solar
        aspectos, pos_natales, pos_revolucion, fecha_revolucion = carta.crear_carta_revolucion_solar(
            ...
            directorio_salida="revoluciones_solares_generadas"
        )
        ...
        print("\n✓ Carta de revolución solar generada exitosamente")
...
def generar_revoluciones_multiples_años():
    """Generar revoluciones solares para múltiples años"""
    print("=== GENERADOR DE REVOLUCIONES SOLARES MÚLTIPLES AÑOS ===")
    ...
    for i, año in enumerate(años_revolucion, 1):
        try:
            print(f"\n--- Generando revolución solar {i}/{len(años_revolucion)} ---")
            print(f"Año: {año}")
            ...
            aspectos, pos_natales, pos_revolucion, fecha_revolucion = carta.crear_carta_revolucion_solar(
                ...
                ciudad_revolucion=ciudad_revolucion,
                guardar_archivo=True,
                directorio_salida="revoluciones_multiples"
            )
            
            print(f"✓ Revolución Solar {año} completada - Aspectos encontrados: {len(aspectos)}")
    ...
    print("\n✓ Todas las revoluciones solares generadas exitosamente")

# FUNCIÓN PERSONALIZADA PARA SOFIA - CORREGIDA CON DATOS DINÁMICOS
def generar_revolucion_solar_personalizada(datos_usuario=None, nombre_archivo_salida=None):
    """Generar revolución solar con datos REALES del usuario"""
    print("=== GENERADOR DE REVOLUCIÓN SOLAR PERSONALIZADO ===")
    ...
    try:
        ...
        print(f"✓ Revolución Solar personalizada generada exitosamente")
