...
def enviar_a_make(datos):
    try:
        url = os.getenv("MAKE_WEBHOOK_ASTROLOGA_ASTROLHORARIA")
        ...
        if not url:
            print("❌ ERROR: No hay URL de Make configurada para MAKE_WEBHOOK_ASTROLOGA_ASTROLHORARIA")
            print("Verifica que MAKE_WEBHOOK_ASTROLOGA_ASTROLHORARIA esté en tu archivo .env")
            return False
        ...
        headers = {
            ...
            'User-Agent': 'AS-Asesores-Astrologa-Astrolhoraria/1.0'
        }
...
def handle_astrologa_astrolhoraria_webhook(data):
    try:
        session_id = data.get("session_id")
        mensaje_usuario = data.get("text", "")

        print(f"=== WEBHOOK ASTROLOGA ASTROLHORARIA ===")
        ...
        print(f"Datos actuales (Astrologa - Astrolhoraria): {datos}")
        ...
        if completos:
            ...
            if exito_envio:
                # Generar informe de astrología horaria
                resumen = f"Resumen completo de 15 minutos sobre astrología horaria con todos los datos proporcionados durante la consulta."
                resultado = generar_y_enviar_informe_desde_agente(data, 'astrologia_horaria_ia', resumen)
        ...
        print(f"❌ Error en handle_astrologa_astrolhoraria_webhook: {e}")
        import traceback
        traceback.print_exc()
