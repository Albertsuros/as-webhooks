...
def completar_datos(session_id, mensaje):
    ...
    # ✅ CORRECCIÓN 2: Detectar servicios lectura facial
    servicio_detectado = None
    
    if "lectura" in mensaje_lower or "facial" in mensaje_lower:
        servicio_detectado = "Lectura Facial"
        ...
        servicio_detectado = "Lectura Facial"  # Por defecto
...
def enviar_a_make(datos):
    try:
        url = os.getenv("MAKE_WEBHOOK_LECTURA_FACIAL")
        
        # ✅ CORRECCIÓN 3: Debugging mejorado
        print(f"=== DEBUG ENVIAR_A_MAKE (LECTURA FACIAL) ===")
        print(f"URL desde .env: {url}")
        print(f"Datos a enviar: {datos}")
        
        if not url:
            print("❌ ERROR: No hay URL de Make configurada para MAKE_WEBHOOK_LECTURA_FACIAL")
            print("Verifica que MAKE_WEBHOOK_LECTURA_FACIAL esté en tu archivo .env")
            return False
        ...
        headers = {
            ...
            'User-Agent': 'AS-Asesores-Lectura-Facial/1.0'
        }
...
def responder_ia(mensaje_usuario, datos_actuales=None):
    try:
        ...
        prompt = f"""Eres Sofía, la lectora de facial virtual de AS Cartastral. Atiendes llamadas entrantes con una voz cálida y cercana para recoger los datos necesarios para generar una carta astral o interpretación personalizada.
...
def handle_lectura_facial_webhook(data):
    try:
        session_id = data.get("session_id")
        mensaje_usuario = data.get("text", "")

        print(f"=== WEBHOOK LECTURA FACIAL ===")
        ...
        print(f"Datos actuales (Lectura - Facial): {datos}")
        ...
        if completos:
            ...
            if exito_envio:
                # PRIMERO: Generar informe
                resumen = f"Resumen completo de 15 minutos sobre lectura facial con todos los datos proporcionados durante la consulta."
                resultado = generar_y_enviar_informe_desde_agente(data, 'lectura_facial_ia', resumen)
                
                # DESPUÉS: Eliminar sesión
                sessions.pop(session_id, None)
                
                return {"type": "speak", "text": "Gracias. Ya tengo toda la información. En breve recibirás tu interpretación y tu informe completo por email."}
            else:
                return {"type": "speak", "text": "Gracias por la información. Estamos procesando tu solicitud lectura facial y nos contactaremos contigo pronto."}
        else:
            respuesta = responder_ia(mensaje_usuario, datos)
            return {"type": "speak", "text": respuesta}

    except Exception as e:
        print(f"❌ Error en handle_lectura_facial_webhook: {e}")
        import traceback
        traceback.print_exc()
